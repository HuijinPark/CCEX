...
void BathSpin_copy(BathSpin* dest, const BathSpin* src, int nqubit) {
    // 기본 필드 복사 (포인터 제외)
    strncpy(dest->name, src->name, MAX_CHARARRAY_LENGTH);
    dest->spin     = src->spin;
    dest->gyro     = src->gyro;
    dest->state    = src->state;
    dest->detuning = src->detuning;
    dest->disorder = src->disorder;
    dest->mindist  = src->mindist;
    dest->mainspidx = src->mainspidx;
    for (int i = 0; i < 3; i++) dest->xyz[i] = src->xyz[i];

    // Matrix 필드 복사
    dest->quad      = src->quad;
    dest->hypf_sub  = src->hypf_sub;

    // 기존 hypf가 할당돼 있으면 해제 (예방)
    //if (dest->hypf != nullptr) {
    //    delete[] dest->hypf;
    //}
    //printf("\n");
    //printf("\n");
    //printf("\n");
    dest->hypf = new MatrixXcd[nqubit];
    for (int j = 0; j < nqubit; j++) {
        dest->hypf[j] = src->hypf[j];
    }
}

void BathArray_setBath_i(BathArray* ba, const BathSpin* bath, int i, int nqubit) {
    int nspin = BathArray_getNspin(ba);
    if (bath == nullptr || i >= nspin) {
        fprintf(stderr,"Error(BathArray_setBath_i): bath couldn't be NULL or out of range\n");
        exit(EXIT_FAILURE);
    }

    // 기존 방식 대신 구조체 단위로 복사
    BathSpin_copy(ba->bath[i], bath, nqubit);
}
...
//    std::cout << "nqibut : " << nqubit << std::endl;
//    for (int j=0; j<nqubit; j++){
//        std::cout << "[DEBUG] : Before j (nqubit idx) = " << j << std::endl;
//        BathArray_setBath_i_hypf_j(ba,bath->hypf[j],i,j);
//        std::cout << "[DEBUG] : After j (nqubit idx) = " << j << std::endl;
//        std::cout << "[DEBUG] : hypf \n" << bath->hypf[j] << std::endl;
//        std::cout << "[DEBUG] : ba->bath[i]->hypf[j] \n" << ba->bath[i]->hypf[j] << std::endl;
//        printf("\n");
//    }
//}
