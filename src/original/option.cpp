...
void cJSON_readOptionPulse(Pulse* pulse, char* fccein) {
    ...
    Pulse_allocAxes(pulse);             // pulse_axes = char* [MAX_CHARARRAY_LEGHT(20)] (npulse+1)
    ...
    for (int i = 0; i < npulse+1; ++i) {
        if (pulse->pulse_axes[i] == NULL) {
            printf("malloc failed at i = %d\n", i);
        } else {
            printf("pulse->pulse_axes[%d] allocated\n", i);
        }
    }
    ...
    for (int i = 0; i < (pulse->npulse)+1; ++i) {
        printf("pulse->pulse_axes[%d] = %s\n", i, pulse->pulse_axes[i]);
    }
    ...
    //         strcpy(pulse->pulse_axes[i], axis->valuestring); // axis
