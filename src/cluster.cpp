...
void Cluster_clusterize(Cluster* cls, BathArray* ba, Config* config){
    ...
    //
    // cls.clusinfo[i][0][0] = the number of cluster at i-th order with adding 1
    // cls.clusinfo[i][j][0] = (L)^p
    // --
...
int Cluster_setClusinfo_addcluster(Cluster* cls, int order, int iter, int* cluster){
    ...
    std::cout << "[DEBUG] ncluster_old : " << ncluster_old << std::endl;
    std::cout << "[DEBUG] ncluster_new : " << ncluster_new << std::endl;
    std::cout << "[DEBUG]           ic : " << ic << std::endl;
    std::cout << "[DEBUG]      cluster : " << *cluster << std::endl;
    std::cout << "[DEBUG]        order : " << order << std::endl;
