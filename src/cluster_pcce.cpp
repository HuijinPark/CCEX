...
///////////////////////////////////
void BathArray_connectivity_pcce(int*** cmap, float*** stmap, Point* centers, float rdip, float rdipcut, int ClusterNum){
//void BathArray_connectivity_pcce(int*** cmap, float*** stmap, Point* centers, float rdip, float rdipcut, int TotalSpinNum){
     
    //Point* centers = ba->centers;
    // cmap [ClusterNum][ClusterNum] = 1 if connected else 0
    // stmap [ClusterNum][ClusterNum] = strength of the connection

    // Connectivity Map and Strength Map
    *cmap  =   allocInt2d(ClusterNum, ClusterNum);
    *stmap = allocFloat2d(ClusterNum, ClusterNum);
    ...
    for (int sp1=0; sp1<ClusterNum; sp1++){
        for (int sp2=sp1+1; sp2<ClusterNum; sp2++){
