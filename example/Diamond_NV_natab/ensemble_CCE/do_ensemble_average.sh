...
data_filename_format="CCE2_Diamond_NV_natab_CONF_ErrCorr"
...
# Nan to num                : -ntn
python ${CodePath} -d "${data_directory}" -fi "${data_filename_format}" -vn "${variable_name1}" -v ${variable_values1} -uc "ms" -ea -ntn -fo "${result}" -pl -fit -fit_initp 2 -fit_space 5 #-vb
...
# Unit conversion           : -uc [unit] (use default unit (ms))
# Nan to num                : -ntn
#python ${CodePath} -d "${data_directory}" -fi "${data_filename_format}" -vn "${variable_name1}" -v ${variable_values1} -uc "ms" -ea -ntn -fo "${result}"
